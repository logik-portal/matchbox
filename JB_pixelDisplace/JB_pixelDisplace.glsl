...
//	st2.y =( gl_FragCoord.y - (((ry -ryi)*getDispInput1.x + (by - byi) * getDispInput1.z + (gy - gyi) * getDispInput1.y)*scanlineY)+ offsetY) / adsk_result_h;
	...
	// bools won't autocast to int anymore and casting with (int) seems
	// to confuse the shader compiler in 2025+

	int useMatte = useMattebool ? 1 : 0;

	outColor = (((getColorInputDisp*(1+(useMatte*(getColorInputDispMap-1)))) + (getColorInputClean*(useMatte*(1-getColorInputDispMap))))*(1-blender))+ (getColorInputClean*blender);
