...
void main()
{
    ...
    if (chromaticOn && (abs(redScale) > 0.001 || abs(blueScale) > 0.001 || chromaticBlur > 0.001))
    {
        ...
        // Use smoothstep to fade out chromatic aberration near edges to prevent artifacts
        vec2 edgeDistance = min(coords, vec2(1.0) - coords);
        float edgeFade = smoothstep(0.0, 0.05, min(edgeDistance.x, edgeDistance.y));
        
        // Blend between original and scaled coordinates based on edge distance
        // Use more gradual fade to reduce flickering
        redCoords = mix(coords, redCoords, edgeFade * edgeFade);
        blueCoords = mix(coords, blueCoords, edgeFade * edgeFade);
