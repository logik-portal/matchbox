...
void main(void) {
	...
	float matte = texture2D(INPUT2, st).r;
	front = do_colorspace(front, 0);
	vec3 lum = vec3(luma(front));
	front = mix(lum, front, saturation);
  front = pow(front, vec3(1.0 - (gamma - 1.0)));

	//vec3 thresh = step(threshold, front);
  vec3 thresh = smoothstep(threshold, 1.0, front);

  if (invert_threshold) {
    thresh = step(threshold, vec3(1.0) - front);
  }

	vec3 bw = vec3(luma(thresh));
